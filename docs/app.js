...
// ── Strings (all UI text, both languages) ────────────────────────────────────
const STR = {
  he: {
...
(async function boot() {
  ...
  document.getElementById('lang-toggle').addEventListener('click', () => {
    ...
    applyLang();
    if (window._mvpData) render(window._mvpData);
  });
...
function renderTrend(d) {
  ...
  const statusColor = { red: '#ef4444', yellow: '#f59e0b', green: '#10b981' };
  const color = statusColor[d.status] || '#10b981';
...
function renderSources() {
  ...
  const groups = [
    { key: 'en', label: t('sources_en'), names: SOURCES.en },
    { key: 'he', label: t('sources_he'), names: SOURCES.he },
    { key: 'fa', label: t('sources_fa'), names: SOURCES.fa },
  ];
